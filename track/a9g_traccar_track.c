 ...
 */
#define SERVER_IP    "ss.neucrack.com" // IP адрес сервера или доменное имя
...
void gps_testTask(void *pData)
{
    ...
    while(1)
    {
        if(isGpsOn)
        {
            ...
            isDataSend = false; // Флаг отправки сообщения (false - "флаг опущен")
            // ЕСЛИ: начальное состояние счетчика циклов -1 или 120 - отчет оправить, обнулить счетчик циклов.
            if((loop_counter < 0)||(loop_counter > 120)){
                // отправка...
            ...
            // ЕСЛИ: скорость более 5 км/ч, и прошло 50 цикла, и "флаг опущен" - отчет оправить, обнулить счетчик циклов;
            if((speed > 5.0)&&(loop_counter > 50)&&(!isDataSend)){
                // отправка...
                if(Http_Post(SERVER_IP,SERVER_PORT,requestPath,NULL,0,buffer,sizeof(buffer)) < 0){
                    Trace(1,"send location to server fail");
                    isDataSend = false;
            ...
            // ЕСЛИ: скорость более 25 км/ч, и прошло 5 цикла, и "флаг опущен" - отчет оправить, обнулить счетчик циклов;
            if((speed > 25.0)&&(loop_counter > 5)&&(!isDataSend)){
                // отправка...
                if(Http_Post(SERVER_IP,SERVER_PORT,requestPath,NULL,0,buffer,sizeof(buffer)) < 0){
                    Trace(1,"send location to server fail");
                    isDataSend = false;
            ...
            // ЕСЛИ: скорость более 50 км/ч и "флаг опущен" - отчет оправить, обнулить счетчик циклов;
        ...
        OS_Sleep(5000);    // СОН ЗАДАЧИ 5 секунды, но A9G в работе!
    }
}
